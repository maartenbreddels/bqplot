...
describe("coloraxis >", () => {
    ...
    it("axis add/remove", async function() {
        ...
        const scale_color = await create_model_bqplot(this.manager, 'ColorScale', 'color_scale_1', {'colors': ['#f00', '#f00'], min: 0, max:1})
        const x_axis = await create_model_bqplot(this.manager, 'Axis', 'axis_model_x', {'side': 'left', 'scale': scale_x.toJSON()})
        const color_axis = await create_model_bqplot(this.manager, 'ColorAxis', 'coloraxis_model_x', {'side': 'right', 'scale': scale_color.toJSON()})
